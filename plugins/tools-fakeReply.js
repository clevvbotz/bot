...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  if (!text) return m.reply(`Harap menggunakan perintah\n\n*${usedPrefix + command}* Halo @${m.sender.split`@`[0]} bagaimana kabarmu?`, null, { mentions: [m.sender] })
  let cm = copy(m)
  let who
  if (text.includes('@0')) who = '0@s.whatsapp.net'
  else if (m.isGroup) who = cm.participant = m.mentionedJid[0]
  else who = m.chat
  if (!who) return m.reply(`Harap menggunakan perintah\n\n*${usedPrefix + command}* Halo @${m.sender.split`@`[0]} bagaimana kabarmu?`, null, { mentions: [m.sender] })
  cm.key.fromMe = false 
